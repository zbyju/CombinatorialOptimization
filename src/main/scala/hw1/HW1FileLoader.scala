...
import common.file.instancefiles.decision.DecisionInstanceFile
...
case class HW1FileLoader() extends FileLoader {
  ...
  val instanceFilesNR = filenamesNR.zipWithIndex.map(f => new DecisionInstanceFile(f._2, pathNR + f._1, f._1, getInputLines(getInputFile(f._1)).toList))
  val instanceFilesZR = filenamesZR.zipWithIndex.map(f => new DecisionInstanceFile(f._2, pathZR + f._1, f._1, getInputLines(getInputFile(f._1)).toList))
