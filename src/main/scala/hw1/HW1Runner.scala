...
import common.knapsack.solver.decision.{BBDecisionKnapsackSolver, DecisionKnapsackSolver}
...
object HW1Runner extends Runner(1) {
  ...
  def runInstanceById(file: InstanceFile, instanceId: Int, solver: AbstractDecisionKnapsackSolver): DecisionResult = {
    solver.solve(file.instances(instanceId), new StatsTracker())
  }

  def runInstancesByFile(file: InstanceFile, solver: AbstractDecisionKnapsackSolver): Seq[DecisionResult] = {
    file.instances.map(i => solver.solve(i, new StatsTracker()))
  }

  def runAllFiles(instanceFiles: Seq[InstanceFile], solver: AbstractDecisionKnapsackSolver): Seq[Seq[DecisionResult]] = {
    instanceFiles.map(f => f.instances.map(i => solver.solve(i, new StatsTracker)))
  }

  def runAllFiles(maxIndex: Int, instanceFiles: Seq[InstanceFile], solver: AbstractDecisionKnapsackSolver): Seq[Seq[DecisionResult]] = {
    instanceFiles.zipWithIndex.filter(f => f._2 <= maxIndex)
      .map(f => f._1.instances.map(i => solver.solve(i, new StatsTracker)))
  }

  def runAllFiles(minIndex: Int, maxIndex: Int, instanceFiles: Seq[InstanceFile], solver: AbstractDecisionKnapsackSolver): Seq[Seq[DecisionResult]] = {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    println("BBZR Done")
    FileSaver.saveFileDecisionResults(statsNR, "NR", 1)
    FileSaver.saveFileDecisionResults(statsZR, "ZR", 1)
    FileSaver.saveFileDecisionResults(statsBBNR, "BBNR", 1)
    FileSaver.saveFileDecisionResults(statsBBZR, "BBZR", 1)
  }
}
